...
function sanitize(payload: any): Payload | null {
  ...
  return { p_first_name, p_last_name, p_phone, p_email, p_roles: roles };
}
...
export async function POST(req: Request) {
  ...
  if (!payload || !payload.p_first_name?.trim()) {
    return NextResponse.json({ error: "First name is required." }, { status: 422 });
  }

  const { data, error } = await supabase.rpc("people_insert_with_roles_v2", payload as any);
  if (error) {
