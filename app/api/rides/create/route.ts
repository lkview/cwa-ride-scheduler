...
export async function POST(req: Request) {
  ...
  const required: (keyof RideCreate)[] = ["date", "ride_time", "pickup_location_id", "emergency_contact_id", "pilot_id", "passenger1_id"];
  for (const f of required) {
    if (!(body as any)[f]) return NextResponse.json({ error: `Missing ${String(f)}` }, { status: 400 });
  }
  ...
  const insert = {
    ...
    status: toDbStatusLower(body.status),
