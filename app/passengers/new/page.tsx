...
export default function NewPassengerPage() {
  ...
  return (
    <div className="max-w-xl mx-auto p-6">
      <h1 className="text-xl font-semibold mb-4">New Passenger</h1>
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <input className="border rounded px-3 py-2" placeholder="Phone (optional)" value={form.phone} onChange={f('phone')} />
          <input className="border rounded px-3 py-2" type="email" placeholder="Email (optional)" value={form.email} onChange={f('email')} />
        </div>
        ...
        <p className="text-sm text-gray-500">* Either phone or email is required.</p>
        <button disabled={busy} className="px-3 py-2 rounded bg-black text-white disabled:opacity-60">
          {busy ? 'Saving…' : 'Create Passenger'}
        </button>
      </form>
