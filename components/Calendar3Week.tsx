...
/**
 * Calendar3Week
 * - Renders the next 3 weeks (21 days) starting today.
 * - Fetches rides from /api/rides/list (uses the SQL view ride_event_summary_v).
 * - Each day cell shows minimal info (time pills). Hover a pill to see full details.
 */
type Ride = {
  id: string;
...
function fromISO(iso: string): Day {
    ...
    <div className="border rounded-lg p-3 bg-white shadow-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-xs text-gray-500">
          Minimal view — hover a time to see details
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, idx) => {
          ...
          return (
            <div
              key={key}
              className={`relative min-h-[92px] border rounded p-2 bg-gray-50 ${isToday ? "ring-2 ring-blue-400" : ""}`}
            >
              {/* date label */}
